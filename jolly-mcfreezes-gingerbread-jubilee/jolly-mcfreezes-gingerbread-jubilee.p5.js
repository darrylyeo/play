...
var _3D = Program._3D = (Program._3D && Program._3D.isSetup && optimizeRestarts) ? Program._3D : (function(){
	var _3D = createGraphics(width, height, WEBGL);
	...
	var _3D_Extension = {
		...
		// Camera calculation
		camera: {
			center: createVector(1200, 1200, 200),
			rotateZ: 46,
			rotateXY: 101,
			...
			try {
				...
				if(keyIsPressed){
					switch(keyCode){
						...
						case RIGHT:
						case DOWN:
							var moveBy = createVector(0, options.walkSpeed);
...
var snowNoes = loadImage("https://www.kasandbox.org/third_party/javascript-khansrc/live-editor/build/images/seasonal/snownoes.png");
var fatherWinston = loadImage("https://www.kasandbox.org/third_party/javascript-khansrc/live-editor/build/images/seasonal/father-winston.png");
var hopDeer = loadImage("https://www.kasandbox.org/third_party/javascript-khansrc/live-editor/build/images/seasonal/hopper-reindeer.png");
			...
			this.open();
				...
				if(options.showIcing){
					...
					for(var i = 0; i <= 1; i += (!isSide ? 1/3 : 2)){
						this.open();
							...
							house.icing(
								createVector(-width/2 - house.roofOverhang, (length/2 + house.roofOverhang) * (1 - i), 0),
								createVector(width + house.roofOverhang * 2, undefined)
							);
						this.close();
					}
					if(isSide){
						var roofTip = createVector(0, length/2 + house.roofOverhang, house.roofHeight);
						var halfRoofWidth = width/2 + house.roofOverhang;
						house.icing(roofTip, createVector(-halfRoofWidth, 0, -house.roofHeight), 7, 22);
						house.icing(roofTip, createVector(halfRoofWidth, 0, -house.roofHeight), 7, 22);
					}
					this.close();
				}
				...
				if(a === 1){
					...
					this.open();
						...
						if(options.showIcing){
							this.fill(255);
							var topLeft = createVector(0, house.depth * 0.2 + 15, 175);
							var topRight = createVector(0, -house.depth * 0.2 - 15, 175);
							var doorHeight = createVector(0, 0, -350);
...
var Snowflake = function(z){
	this.pos = createVector(random(-1000, 1000), random(-1000, 1000), z || random(1300));
	this.vel = createVector(random(-1, 1) * options.snowflakeMaxHorizontalVelocity, random(-1, 1) * options.snowflakeMaxHorizontalVelocity, random(-options.snowflakeMaxVerticalVelocity, -1));
	this.rotate = createVector(random(TWO_PI), random(TWO_PI), random(TWO_PI));
	this.rotateVel = createVector(random(TWO_PI*0.1), random(TWO_PI*0.1), random(TWO_PI*0.1));
};
