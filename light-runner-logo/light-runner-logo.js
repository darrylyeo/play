...
draw = function() {
	...
	for(var x = -frameCount % (s * 2); x <= width + 40 * width / 400; x += s){
		for(var y = -width / 400 - s * 4; y <= height; y += s){
			rect(x - y / 10, y + x / 10, 2 *  width / 400, 2 * width / 400);
		}
	}
	
	lightRunnerLogo(width / 2, height / 2, min(width, height) / 400);
