// Featured on darryl-yeo.com: darryl-yeo.com/buzy-buzzy-bee
...
var Bee = function(x, y){
	...
	this.fromPosition = createVector(x, y);
	//How far away the bee's target position is
	this.targetOffset = createVector(0, 0);
	...
	for(var a = 0; a < 360; a += 2){
		...
		for(var h = 0; h < 4; h++){
			hairStrand.vertices.push(createVector(random(-3, 3), random(-3, 3)));
		}
...
Bee.prototype.update = function(){
	...
	this.flying = this.onFlower ? (this.position.dist(createVector(this.fromPosition.x + this.targetOffset.x, this.fromPosition.y + this.targetOffset.y)) > 20) : true;
...
p5.mouseClicked(function(){
	if(selectedFlowerNumber){
		...
		bee.goTo(createVector(flower.position.x + flower.horizontalOffset(), flower.top()));
	}else{
		//Otherwise, just go to the mouse's coordinates.
		bee.goTo(createVector(mouseX, mouseY));
	}
